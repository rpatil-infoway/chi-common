...
//	}
	...
	function submitClinReqSpace(e, state) {
...
//		state.pageData.name = state.pageData.name  + " (" + state.pageData.jurisdictionKey + ")";
		...
		state.pageData.ContentPageTitle = AJS.I18n.getText("confluence.clinicalreq.space.blueprint.homepage.suffix");
//        console.log(state);
//        console.log(e);
...
//		return false;
	...
	function postRenderClinReqSpace(e, state) { 
    ...
    function preRenderClinReqSpace(e, state) {
    ...
    Confluence.Blueprint.setWizard('ca.infoway.confluence.plugins.chi-common:chi-clin-req-space-blueprint-item', function(wizard) {
    	wizard.on("pre-render.spaceBasicDetailsId", preRenderClinReqSpace);
        wizard.on("post-render.spaceBasicDetailsId", postRenderClinReqSpace);
        wizard.on("submit.spaceBasicDetailsId", submitClinReqSpace);
    });
